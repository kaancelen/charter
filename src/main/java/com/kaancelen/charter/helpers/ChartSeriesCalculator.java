...
public class ChartSeriesCalculator {
	/**
	 * TCMB nakit/gayrinakit risk toplamlar�n� hesaplar
	 * @param records
	 * @param nakit = true nakit hesab�, nakit = false gayrinakit hesab� 
	 * @return
	 */
	...
	/**
	 * Sekerbank nakit/gayrinakit risk toplamlar�n� hesaplar
	 * @param records
	 * @return
	 */
	public static ChartSeries sekerbankNakitRisk(List<Record> records, boolean nakit){
		...
		ChartSeries sekerbankNakitRisk = new ChartSeries("Şekerbank "+(nakit?"Nakit":"Gayrinakit")+" Risk");
		sekerbankNakitRisk.setData(sekerbankNakitRiskMap);
		return sekerbankNakitRisk;
	}
	
	/**
	 * Her d�nem nakit/gayrinakit risk kay�d� bulunan kurum say�s�
	 * @param records
	 * @return
	 */
	...
	/**
	 * TCMB nakit/gayrinakit limit toplamlar�n� hesaplar
	 * @param records
	 * @return
	 */
	...
	/**
	 * TCMB kisa vadeli nakit/gayrinakit risk toplam�
	 * @param records
	 * @return
	 */
	...
	/**
	 * TCMB orta vadeli nakit/gayrinakit risk toplam�
	 * @param records
	 * @return
	 */
	...
	/**
	 * TCMB uzun vadeli nakit/gayrinakit risk toplam�
	 * @param records
	 * @return
	 */
	...
	/**
	 * TCMB nakit teminine y�nelik teminat mektubu (201, 251)
	 * @param records
	 * @return
	 */
	...
	public static ChartSeries DepartmentReport(List<JobRecord> jobRecords, int type) {
		...
		for (JobRecord jobRecord : jobRecords) {
			...
			case 3:
				//�ek
				if(jobRecord.getType() != null && jobRecord.getType().contains(BankConstants.cek)){	//Toplam �ek
					departmentMap.put(ChartConstants.DEPARTMENT_LABELS[3], 1 + (Integer)departmentMap.get(ChartConstants.DEPARTMENT_LABELS[3]));
					if(jobRecord.getResult() != null && !jobRecord.getResult().contains(BankConstants.olumsuz)){ //olumlu �ek
						departmentMap.put(ChartConstants.DEPARTMENT_LABELS[4], 1 + (Integer)departmentMap.get(ChartConstants.DEPARTMENT_LABELS[4]));
					}
					if(jobRecord.getResult() != null && jobRecord.getResult().contains(BankConstants.olumsuz)){ //olumsuz �ek
