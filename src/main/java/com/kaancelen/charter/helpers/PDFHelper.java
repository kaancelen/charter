...
public class PDFHelper {
	...
	public static void createMemzucReport(List<List<Map<Object, Number>>> datas){
		try {
			...
			createPage(document, FileConstants.CHART_3, datas.get(3), Arrays.asList("Faktoring", "Leasing"));
	...
	private static void createPage(PDDocument document, String imagePath, List<Map<Object, Number>> data, List<String> rowLabels) throws IOException{
		int heightBuffer = 300;//image buffers
		int widthBuffer = 100;
		//convert image
		...
		contentStream.drawImage(image, widthBuffer/2, 4*heightBuffer/5);
		//draw table
		if(imagePath.compareTo(FileConstants.DEPT_CHART) == 0)
			drawTable(page, contentStream, 3*heightBuffer/5, widthBuffer/2, convertDataToStringWithoutRowHeaders(data, rowLabels));
		else
			drawTable(page, contentStream, 3*heightBuffer/5, widthBuffer/2, convertDataToString(data, rowLabels));
		//save file
		contentStream.close();
	}
	 ...
	 * @return
	 */
	private static String[][] convertDataToStringWithoutRowHeaders(List<Map<Object, Number>> data, List<String> labels){
		String[][] content = new String[data.size()+1][data.get(0).size()];
		System.out.println("content["+data.size()+1+"]["+data.get(0).size()+"]");
		//Set column headers
		List<Object> keys = new ArrayList<Object>(data.get(0).keySet());
		for(int i=0; i< data.get(0).size(); i++){
			content[0][i] = (String) keys.get(i);//set column header
		}
	...
	private static String[][] convertDataToString(List<Map<Object, Number>> data, List<String> labels){
		...
		for(int i=1; i< data.size()+1; i++){//row
			...
			for (Entry<Object, Number> entry : map.entrySet()){
				if(entry.getValue() instanceof Double){
					if((Double)entry.getValue()/100 < 1 && (Double)entry.getValue()/100 > 0){
	...
	private static void drawTable(PDPage page, PDPageContentStream contentStream,
	    ...
	    PDFont data = PDType1Font.getStandardFont("TAHOMA");
	    int fontSize = 11;
	    if(cols > 8)
	    	fontSize = 10;
	    else if(cols > 12)
	    ...
	    for(int i = 0; i < content.length; i++){
	        for(int j = 0 ; j < content[i].length; j++){
	        	...
	        	if(i==0 || j==0){
	        		contentStream.setFont(header, fontSize-1);
	        	}else{
